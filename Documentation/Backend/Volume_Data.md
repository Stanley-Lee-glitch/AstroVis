...
| `FieldHierarchy.get_info()` | Summary dict of block count and cell size per level |
...
for b in hierarchy.levels[finest_level].blocks:
    print(b.block_id, b.dims)

hierarchy.get_info()
# {0: {'num_blocks': 8, 'cell_size': array([...])}, 1: {...}, ...}
```

### ```preview_field_slice(data, fields=None, axis=0, percentile=50.0, slice_pos=None, levels=None, output_path="preview.png", log=False, value_ranges=None)```

Plots a `GridBlock` or `FieldHierarchy` sliced along `axis` at a given position (or percentile of the domain extent), with an image + value histogram per field.

It is useful for a check before exporting.

- `log=True` applies `log10` scaling to both the image values and the histogram values, which is useful for density-like fields that span many orders of magnitude.
- `value_ranges` lets you pin a fixed `(min, max)` plotting range, either for all fields or per field, so repeated calls keep the same color scale across frames.

```python
preview_field_slice(hierarchy, fields="density", axis=0, percentile=50, output_path="density_check.png")
```
...
preview_field_slice(
    ...
    axis=0,
    percentile=50,
    output_path="density_check.png",
    log=True,
    value_ranges={"density": (-8.0, 0.0)},
)
```

### ```get_field_value_ranges(data, fields=None, log=False) -> dict[str, tuple[float, float]]```

Returns the true global `(min, max)` range for one or more fields across the full `GridBlock` or `FieldHierarchy`.

Use this before repeated previews or Blender shading when you want one consistent range across many frames.

```python
ranges = get_field_value_ranges(hierarchy, fields="density", log=True)
print(ranges["density"])
```

### ```export_volume_sequence(input_dir, output_dir, field="density", vtype="gas", preview_every=10, log=True) -> dict```

Loads a snapshot directory once, exports all frames to partitioned VDB folders, saves preview images every `preview_every` frames, and reports the final global field range for Blender.

This is the simplest path when you want:
...
It's useful for inspecting a dataset's grid layout more detail on its own.
