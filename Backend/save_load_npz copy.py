    ...
    for name, num_frames in volume_list.items():
        ...
        for i in range(num_frames):
            ...
            for k in data.files:
                ...
                if 'cell_size' in k:
                    temp_levels[level_id]['cell_size'] = np.array(data[k])
                    continue
                ...
                if 'block' in key_core:
                    ...
                    if 'left_edge' in key_core:
                        block_dict['left_edge'] = np.array(data[k])
                    elif 'right_edge' in key_core:
                        block_dict['right_edge'] = np.array(data[k])
                    elif 'dims' in key_core:
                        block_dict['dims'] = np.array(data[k])
